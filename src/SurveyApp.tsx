...
const SurveyApp = () => {
    const [stage, setStage] = useState<'intro' | 'survey' | 'result'>('intro')
    ...
    const getResultType = (score: number): ResultType => {
        if (score <= 25) return { label: "개방형", desc: "다양한 관점을 존중하며 성평등 이슈에 개방적인 태도를 보입니다. 대화와 이해를 중시하는 성향입니다." }
        if (score <= 50) return { label: "신중형", desc: "성평등 이슈에 신중하게 접근하며 균형잡힌 시각을 유지하려 노력합니다. 상황에 따라 유연한 판단을 보입니다." }
        if (score <= 75) return { label: "방어형", desc: "성평등 이슈에 경계심을 갖고 있으며 역차별에 대한 우려가 있습니다. 현재 구조에 대한 불만을 표현합니다." }
